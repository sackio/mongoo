...
module.exports = function(schema, options){
  ...
  //schema.index(_.object([a.o.path + '.coordinates'], ['2d']));
  ...
  if (a.o.geocoder_api){
      ...
      if (_.isArray(this.get(path))){
        ...
        return Async.eachSeries(this.get(path), function(l, cb){
          ...
          return a.o.geocoder_api[b.o.reverse_geocode ? 'reverse_geocode' : 'geocode'].apply(null, args.concat([function(err, geocoding){
            ...
            //set coordinates
            self.set(path + '.' + (index - 1) + '.coordinates', {
              'Longitude': Belt.get(geocoding, 'geometry.location.lng')
            , 'Latitude': Belt.get(geocoding, 'geometry.location.lat')
            });
    ...
    if (a.o.auto_geocode) schema.pre('save', function(next){
      ...
      if (!a.o.array){
        if ((this.isNew &&
        (this.get(a.o.path + '.coordinates.Longitude') || this.get(a.o.path + '.coordinates.Latitude')))
        || this.isModified(a.o.path + '.coordinates.Longitude') || this.isModified(a.o.path + '.coordinates.Latitude')){
          return this.reverse_geocode(a.o.path, function(err){
            if (err) console.error(err);
            return next()
          });
        }
        if ((this.isNew && this.get(a.o.path + '.given_string')) || this.isModified(a.o.path + '.given_string')){
          return this.geocode(a.o.path, function(err){
            if (err) console.error(err);
            return next()
          });
        }
      ...
      return Async.eachSeries(self.get(a.o.path), function(p, cb){
        ...
        if ((self.isNew &&
        (self.get(a.o.path + '.' + (index - 1) + '.coordinates.Longitude')
        || self.get(a.o.path + '.' + (index - 1) + '.coordinates.Latitude')))
        || self.isModified(a.o.path + '.' + (index - 1) + '.coordinates.Longitude')
        || self.isModified(a.o.path + '.' + (index - 1) + '.coordinates.Latitude')){
          return self.reverse_geocode(a.o.path + '.' + (index -1), Belt.cw(cb));

        if ((self.isNew && self.get(a.o.path + '.' + (index -1) + '.given_string'))
           || self.isModified(a.o.path + '.' + (index -1) + '.given_string'))
          return self.geocode(a.o.path + '.' + (index -1), Belt.cw(cb));
