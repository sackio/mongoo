...
module.exports = function(schema, options){
  ...
  var vp2 = schema.virtual(a.o.path + '_prev_buffer');
  vp2.get(function(){
    return this[a.o.virtual_prefix + a.o.path + '_prev_buffer'];
  });
  vp2.set(function(val){
    return this[a.o.virtual_prefix + a.o.path + '_prev_buffer'] = val;
  });
  ...
  schema.method('file_extension', function(){
    var p = this.get('file.file_path');
    if (!p) return;
    return p.split('.').pop();
  });
  ...
  schema.pre('save', function(next){
    var p = this.get(a.o.path + '.file_path');
    if (!p) return next();
    ...
    return FSTK.mkdir(p, Belt.cw(next, 0));
  });
  ...
  schema.pre('save', function(next){
    if (!this.get(a.o.path + '_prev_buffer')) return next();

    var p = this.get(a.o.path + '_prev_buffer');
    this.set(a.o.path + '_prev_buffer', undefined);

    return FSTK._fs.writeFile(this.get(a.o.path + '.file_path'), p, Belt.cw(next, 0));
  });
